    ...
    <div class="product-hero">
        ...
        <div class="product-info reveal-right">
            ...
            <div class="product-meta">
                @if($product->category)
                <div class="meta-row"><span class="lbl">{{ __('front.category') }}</span><span class="val">{{ $locale==='ar' ? $product->category->name_ar : $product->category->name_en }}</span></div>
                @endif
                @if($product->is_featured==1)
                <div class="meta-row"><span class="lbl">{{ __('front.status') }}</span><span class="val" style="color:var(--gold)">{{ __('front.featured_tag') }}</span></div>
                @endif
            </div>
        ...
        <div class="similar-grid">
            ...
            <a href="{{ route('product.show', $item->id) }}" class="sim-card reveal">
                ...
                <div class="sim-card-body">
                    ...
                    @if($item->price??false)
                    <div class="sim-card-price">{{ $item->price }} {{ $locale==='ar' ? ($item->price_unit_ar??'درهم') : ($item->price_unit_en??'MAD') }}</div>
                    @endif
                </div>
            </a>
            @endforeach
