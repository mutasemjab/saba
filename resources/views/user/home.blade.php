...
<section id="about">
    ...
    <div class="about-inner">
        ...
        <div class="about-text reveal-right">
            <div class="section-header" style="text-align:{{ $locale=='ar'?'right':'left' }};margin-bottom:26px;">
    ...
    <div class="best-track-wrap">
        <div class="best-track" id="bestTrack">
            @foreach([$featuredProducts, $featuredProducts] as $group)
                @foreach($group as $fp)
                <a href="{{ route('product.show', $fp->id) }}" class="best-card" style="text-decoration:none;">
                    ...
                    <div class="best-card-body">
                        ...
                        <div class="best-card-desc">
                            {{ $locale=='ar'
                                ? \Str::limit($fp->description_ar, 80)
                                : \Str::limit($fp->description_en, 80) }}
                        </div>
                        <div class="best-card-footer">
                            ...
                            <div class="best-card-price">
                                {{ $fp->price }}
                                {{ $locale=='ar'
                                    ? ($fp->price_unit_ar ?? 'درهم')
                                    : ($fp->price_unit_en ?? 'MAD') }}
                            </div>
                            @endif
...
<section id="menu">
    ...
    <div class="menu-tabs reveal" id="menuTabs">
        <button class="menu-tab active" data-category="">{{ __('front.all') }}</button>
        @foreach($categories as $category)
        <button class="menu-tab" data-category="{{ $category->id }}">
    ...
    <div class="menu-panel active" id="menuPanel">
        ...
        <a href="{{ route('product.show', $product->id) }}" class="menu-item" style="text-decoration:none;">
            ...
            <div class="menu-item-info">
                ...
                <div class="menu-item-desc">
                    {{ $locale=='ar'
                        ...
                        : \Str::limit($product->description_en, 90) }}
                </div>
            ...
            <div class="menu-item-price">
                {{ $product->price }}<br>
                <small>{{ $locale=='ar'
                    ? ($product->price_unit_ar ?? 'درهم')
                    : ($product->price_unit_en ?? 'MAD') }}</small>
            </div>
            @endif
        </a>
        @empty
        ...
        <div class="contact-info reveal-left">
            ...
            <div class="contact-detail">
                ...
                <div>
                    ...
                    <div class="contact-detail-value">
                        <a href="mailto:{{ $settingNav->email ?? '' }}"
                           style="color:inherit;text-decoration:none;">
                            {{ $settingNav->email ?? '' }}
                        </a>
                    </div>
            ...
            <form method="POST" action="{{ route('contact.store') }}">
                ...
                <div class="form-group">
                    ...
                    <textarea name="message"
                              placeholder="{{ __('front.message_placeholder') }}"
...
$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (res) {
                if (!res.products || !res.products.length) {
                    $panel.html('<div class="menu-empty">{{ __("front.no_products") }}</div>');
                    return;
                }
                var html = '';
                res.products.forEach(function (p) {
                    var name      = locale === 'ar' ? p.name_ar      : p.name_en;
                    var desc      = locale === 'ar' ? p.description_ar : p.description_en;
                    if (desc && desc.length > 90) desc = desc.substring(0, 90) + '...';
                    var tag       = p.is_featured == 1
                        ? '<span class="menu-item-tag">{{ __("front.featured_tag") }}</span>' : '';
                    var priceUnit = locale === 'ar'
                        ? (p.price_unit_ar || 'درهم') : (p.price_unit_en || 'MAD');
                    var priceHtml = p.price
                        ? `<div class="menu-item-price">${p.price}<br><small>${priceUnit}</small></div>` : '';
                    html += `
                        <a href="/product/${p.id}" class="menu-item" style="text-decoration:none;">
                            <div class="menu-item-img">
                                <img src="/assets/admin/uploads/${p.photo}" alt="${name}">
                            </div>
                            <div class="menu-item-info">
            ...
            error: function () {
                $panel.html('<div class="menu-empty">{{ __("front.error_loading") }}</div>');
            }
        });
    });
    ...
    // ─────────────────────────────────────────────
    $('#bestTrack')
        .on('touchstart', function () { $(this).css('animation-play-state', 'paused'); })
        .on('touchend',   function () { $(this).css('animation-play-state', 'running'); });
