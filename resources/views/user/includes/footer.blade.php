...
<footer>
    <div class="footer-top">
        <div class="footer-brand">
            <img src="{{ $settingNav && $settingNav->logo ? asset('assets/admin/uploads/' . $settingNav->logo) : asset('assets_front/images/main_logo.png') }}"
                alt="سبأ" class="footer-brand-logo">
            <p>{{ __('front.footer_about') }}</p>
            <div class="footer-social">
                @if ($settingNav->instagram)
                    <a href="{{ $settingNav->instagram }}" target="_blank" class="social-btn"><i
                            class="fab fa-instagram"></i></a>
                @endif
                @if ($settingNav->facebook)
                    <a href="{{ $settingNav->facebook }}" target="_blank" class="social-btn"><i
                            class="fab fa-facebook-f"></i></a>
                @endif
                @if ($settingNav->twitter)
                    <a href="{{ $settingNav->twitter }}" target="_blank" class="social-btn"><i
                            class="fab fa-x-twitter"></i></a>
                @endif
                @if (!empty($settingNav->whatsapp_number))
                    <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $settingNav->whatsapp_number) }}"
    ...
    <div class="footer-bottom">
        <p class="footer-copy">© {{ date('Y') }} <span>{{ __('front.site_name') }}</span> ·
            {{ __('front.all_rights') }}</p>
